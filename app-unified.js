...
// --- SERVER METADATA ---
const SERVER_VERSION = "3.7.0 (Ultimate Cloud Fix)";
...
app.post('/api/register', (req, res) => {
    ...
    db.get("SELECT * FROM users WHERE LOWER(email) = ? AND device_id = ?", [cleanEmail, deviceId], (err, row) => {
        ...
        const myCode = (name || 'USER').toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 5) + "-" + Math.floor(1000 + Math.random() * 9000);
        ...
        db.get("SELECT COUNT(*) as total FROM activations", (e2, acts) => {
            db.get("SELECT SUM(price_paid) as total FROM activations", (e3, income) => {
                db.all("SELECT * FROM users ORDER BY last_seen DESC LIMIT 10", (e4, recent) => {
                    res.json({ totalUsers: users.total, weeklyUsers: users.total, weeklyActivations: acts.total || 0, weeklyIncome: income.total || 0, users: recent });
                });
            });
        });
    });
...
app.get('/api/admin/active-licenses', (req, res) => {
    db.all("SELECT * FROM licenses ORDER BY status DESC", (err, rows) => res.json(rows || []));
});
...
app.post('/api/admin/config', (req, res) => {
    ...
    else res.status(500).json({ error: "Error guardando config" });
});
...
server.listen(PORT, '0.0.0.0', () => {
    console.log(`✅ ULTIMATE CLOUD SERVER V3.7.0 LIVE ON PORT ${PORT}`);
    updateManifest();
});
